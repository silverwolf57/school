...
T2 n;
public:
Closer(T1 m1,T2 n1):m(m1),n(n1){}
bool operator()(T1 p,T1 q){
    if(n(m,p)!=n(m,q))return n(m,p)<n(m,q);
    else return p<q;
}
...
/*
这道题主要是两个问题
1.在sort中调用了构造函数，需要编写构造函数
2.同时因为需要返回Bool值，实际上是用函数对象，先构造后调用()。于是我们需要重载()<并按要求返回
*/