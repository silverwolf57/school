...
40
提示
1) 编写模板的时候，连续的两个 “>”最好要用空格分开，以免被编译器看作是 ">>"运算符。VS可能无此问题，
但是Dev C++和服务器上的编译环境会有这个问题。
比如 vector<vector<int>> 有可能出错，要改成 vector<vector<int> >

2) 在模板中写迭代器时，最好在前面加上 typename关键字，否则可能会编译错。VS可能无此问题，
但是Dev C++和服务器上的编译环境会有这个问题。
*/
#include <iostream>
#include <string>
...
template <class T, class P, class Pred = greater<T> >
class MyMultimap {
private:
    multimap<T, P, Pred> mp;
public:
    typedef typename multimap<T, P, Pred>::iterator iterator;

    void insert(pair<T, P> p) {
        mp.insert(p);
    }

    iterator begin() { return mp.begin(); }
    iterator end() { return mp.end(); }
    iterator find(T key) { return mp.find(key); }
    void clear() { mp.clear(); }

    void Set(T key, P value) {
        pair<iterator, iterator> range = mp.equal_range(key);
