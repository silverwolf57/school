...
def q2(n):
    ...
    '''
    x = balabala
    x=np.arange(n).reshape(-1,1)+np.arange(n).reshape(1,-1)
    return x
...
def q5():
    ...
    '''
    num = np.random.uniform(-1, 1, (1000000, 2))  
    circle = np.sum(np.sum(num**2, axis=1) <= 1) 
    res=4*circle/1000000
    return res
...
class Layout:
    ...
    def recover_layoutText(self):
        ...
        if self.pacman_pos:
            x, y = self.pacman_pos
            grid[x][y] = 'P'
        layout_lines = ["".join(line) for line in grid]
        return "\n".join(layout_lines)
